	...
	with tf.device('/gpu:0'):
		...
		def _build_net(self):
			def build_layers(s,c_names,n_l1,n_l2,w_initializer,b_initializer):
				def noisy_dense(units, input,activation=tf.nn.relu):
						"""
						noisy dense layer
						:param units:
						:param input:
						...
						dense = tf.matmul(input, tf.transpose(w)) + b
				...
				if self.noisy:
					with tf.variable_scope('l1'):
						l1 = noisy_dense(n_l1,s,activation=tf.nn.relu)
					with tf.variable_scope('l2'):
						l2 = noisy_dense(n_l2,l1,activation=tf.nn.relu)
					if self.dueling:
						with tf.variable_scope('Value'):
							self.V =noisy_dense(1,l2,activation=None)
						with tf.variable_scope('Advantage'):
							self.A =noisy_dense(self.n_actions,l2,activation=None)
						with tf.variable_scope('Q'):
							#为了避免最终哦 A被学成Q(跟dqn效果一样)：当V=0时 A=Q,而A每次减去不同的值，不容易变成Q
							out = self.V + (self.A - tf.reduce_mean(self.A,axis=1,keep_dims=True))
					else:
						with tf.variable_scope('Q'):
							out =noisy_dense(self.n_actions,l2,activation=None)
					return out 					
				else:
					with tf.variable_scope('l1'):
